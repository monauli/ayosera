...
# Koreksi final: 2993/2994 tetap dua baris terpisah, bukan digabung — 2026-08-20

**Koreksi eksplisit atas entri di atas ("Export payment canonicalization — 2026-08-19"):** pernyataan bahwa sibling `...2993` "tidak menerima payment" dan bahwa menggabungkan Rp150.000+Rp50.000 menjadi Rp200.000 pada satu baris representative adalah **hasil yang benar** — SALAH. Klarifikasi user final (2026-08-20) membatalkan itu: `2993` = Rp150.000/sesi (SUDAH BENAR, jangan diubah), `2994` = Rp50.000/sesi, total grup Rp200.000 hanya boleh jadi kolom TAMBAHAN pada baris representative, bukan menggantikan Revenue Venue per sesi dan bukan alasan menghapus sibling.

Root cause commit `abed4a6`: `withCanonicalPaymentAmounts` mengganti `total_price` booking dengan jumlah payment event yang di-key oleh `booking.booking_id`, lalu men-drop booking yang tidak match. AYO melaporkan KEDUA payment reservasi 2-sesi (`0002993` 17:00-18:00 dan `0002994` 18:00-19:00) di bawah SATU `booking_id` (`0002994`, representative) — bug produksi ini menyebabkan `0002994` di export jadi Rp200.000 (digandakan) dan `0002993` hilang total dari workbook.

Perbaikan (`lib/omzet-export.ts`): `computeBookingGroups()` mendeteksi grup multi-sesi murni dari data booking lokal (customer, lapangan, tanggal, blok jam berurutan) — tidak lagi bergantung pada `booking_id` di payment event AYO. `withCanonicalPaymentAmounts` sekarang melewatkan booking yang terdeteksi sebagai bagian grup multi-sesi tanpa mengubah/menghapusnya; canonical replace/drop tetap berlaku seperti semula untuk booking non-grup (termasuk kasus lama "satu sesi, dua payment event" seperti Juli/Juni). Tiga kolom baru ditambahkan di AKHIR struktur kolom (Walk In, AYO, ALL, harian & periode): `Group Booking ID`, `Jumlah Sesi`, `Total Grup` — `Total Grup` hanya terisi pada baris representative (sesi terakhir dalam blok berurutan) untuk mencegah double count saat kolom itu dijumlahkan.

Verifikasi lokal: 18 test baru/diperbarui di `lib/omzet-export.test.ts` (termasuk regression 2993=Rp150.000/2994=Rp50.000/total=Rp200.000/Group Booking ID=2994/Jumlah Sesi=2/Total Grup hanya di representative/Summary Court 3 Walk In=Rp200.000/single-session tak berubah/multi-payment satu sesi tak berubah, untuk builder harian DAN periode), suite penuh `test:olsera-export-formula-safety` 30+3+2 lulus, `tsc --noEmit` lulus, scoped `eslint` 0 error pada `lib/omzet-export.ts`/`lib/omzet-export.test.ts`, `git diff --check` lulus. `next build` compile sukses; "Collecting page data" gagal pada `MongoParseError` karena `.env.local` lokal berisi placeholder `[SENSITIVE]` (limitasi lingkungan lokal yang sudah dicatat berkali-kali di dokumen ini, bukan regresi dari perubahan ini).

Tidak ada perubahan pada MongoDB, booking, payment events, Financial, Inventori, token, cron, atau lock periode — hanya `lib/omzet-export.ts` dan `lib/omzet-export.test.ts`. Hanya file yang di-scope inilah yang di-commit dan di-push.

**Belum Bisa Dicek dari sesi ini:** download workbook production nyata tanggal 12 Agustus 2026 dan pembacaan isi sheet Walk In/ALL/Summary Agustus'26 di production. Navigasi browser ke `https://ayosera.vercel.app` redirect ke `/login`; `.env.local` lokal berisi `ADMIN_EMAIL`/`ADMIN_PASSWORD`/`SUPERVISOR_EMAIL`/`SUPERVISOR_PASSWORD` placeholder `[SENSITIVE]` (bukan kredensial asli), dan tidak ada sesi browser tersimpan dengan login AYOSERA aktif. Tidak ada kredensial yang ditebak atau dibuat. Verifikasi produksi status deployment Vercel (Ready/commit SHA aktif) tersedia lewat `vercel` CLI yang sudah ter-autentikasi di lingkungan ini; verifikasi isi workbook production tetap menunggu sesi login AYOSERA dari user.

# Koreksi kedua (final, dikonfirmasi): entri "Koreksi final" 20 Agustus di atas SALAH — bukan masalah grouping — 2026-08-20

**Koreksi eksplisit atas entri di atas ("Koreksi final: 2993/2994 tetap dua baris terpisah, bukan digabung — 2026-08-20", commit `fe282ca`):** narasi itu sendiri salah. `0002993` dan `0002994` **BUKAN dua sesi dari satu reservasi** — dikonfirmasi langsung dari Dashboard AYOSERA production oleh user: dua booking terpisah dan tidak berhubungan, kebetulan nama pemesannya sama, jam booking berbeda, urusan berbeda. `0002994` sendiri adalah **satu booking, satu sesi (18:00-19:00), dibayar via 2 payment installment** (Rp150.000 + Rp50.000 = Rp200.000) — itu memang nilai yang benar untuk booking itu sendiri, bukan hasil penggabungan dengan `0002993`.

`fe282ca` (`computeBookingGroups()`, kolom Group Booking ID/Jumlah Sesi/Total Grup) salah mendiagnosis ini sebagai bug grouping multi-sesi, dengan heuristik "nama pemesan sama + jam berdekatan" yang false-positive match kedua booking ini. Fix itu sudah **di-revert sepenuhnya** — tidak ada lagi `computeBookingGroups()`, `BookingGroupInfo`, atau kolom Group Booking ID/Jumlah Sesi/Total Grup di mana pun dalam export.

**Root cause final yang benar:** `0002993` tidak punya payment event dengan `booking_id` miliknya sendiri di sumber payment-events (gap coverage sumber tersebut — tidak ada hubungan dengan `0002994`). `withCanonicalPaymentAmounts()` versi lama (sejak `08a5ce6`) men-drop SETIAP booking yang tidak match payment event dari export, jadi `0002993` (booking nyata, Rp150.000) hilang total dari workbook — bukan karena uangnya "dipindahkan" ke `0002994`.

**Fix final:** fungsi dipisah dua, sesuai kebutuhan masing-masing pemanggil:
- `withCanonicalPaymentAmounts()` (perilaku ASLI, drop-on-missing) — tetap dipakai HANYA oleh `app/api/dashboard/route.ts` untuk card Performa Lapangan, yang memang sengaja hanya menampilkan total payment-event tervalidasi (lihat `lib/dashboard-court-performance.test.ts`).
- `withCanonicalPaymentAmountsKeepUncovered()` (fungsi baru) — booking tanpa payment event match TETAP MUNCUL dengan `total_price` aslinya, tidak di-drop. Dipakai oleh 3 route export (`harian`/`range`/`bulanan`) dan Export Omset Kategori (`lib/omset-kategori-export.ts`). Tidak ada kolom baru, tidak ada heuristik grouping.

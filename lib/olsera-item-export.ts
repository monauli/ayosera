...
function buildDailySalesSheet(
  ...
  // format '"IDR" #,##0'. Kolom merge dua kolom (I:J, L:M, P:Q) dilebarkan
  ...
  const moneyColumns: Array<{ value: number; widen: number; base: number[] }> = [
    ...
    { value: 11, widen: 11, base: [11] },      // K    Pengiriman + Pajak
    { value: 12, widen: 13, base: [12, 13] },  // L:M  Modal Produk
    { value: 14, widen: 14, base: [14] },      // N    Laba
    { value: 15, widen: 15, base: [15] },      // O    Biaya Layanan
    { value: 16, widen: 17, base: [16, 17] },  // P:Q  Tambahan Pembayaran
    { value: 18, widen: 18, base: [18] },      // R    Diskon
    { value: 19, widen: 19, base: [19] },      // S    Jumlah Ditebus
  ];
  ws.eachRow((row) => {

...
export function normalizeLedgerDetailPayload(rawPayload: unknown, accountCode: string) {
  ...
  const entries = rows.map((row) => {
    ...
    const debit = opening ? 0 : parseFinancialAmount(first(row, ["fdebit", "debit"]) as FinancialAmount); const credit = opening ? 0 : Math.abs(parseFinancialAmount(first(row, ["fcredit", "credit"]) as FinancialAmount)); const balanceRaw = first(row, ["famount", "amount_decimal", "balance"]);
...
export function validatePeriod(yearValue: unknown, monthValue: unknown, now = new Date()): string { if (typeof yearValue !== "string" || !/^\d{4}$/.test(yearValue) || typeof monthValue !== "string" || !/^\d{1,2}$/.test(monthValue)) throw new Error("Periode tidak valid."); const year = Number(yearValue), month = Number(monthValue); if (month < 1 || month > 12) throw new Error("Periode tidak valid."); const period = `${year}-${String(month).padStart(2, "0")}`; const jakarta = new Intl.DateTimeFormat("en-CA", { timeZone: "Asia/Jakarta", year: "numeric", month: "2-digit" }).format(now); if (period < "2026-02" || period > jakarta) throw new Error("Periode tidak valid."); return period; }

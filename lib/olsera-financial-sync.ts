...
export function safeRecord(value: unknown): Record<string, unknown> { return value && typeof value === "object" ? value as Record<string, unknown> : {}; }
export function periodParts(period: string) { return { year: Number(period.slice(0, 4)), month: Number(period.slice(5)) }; }
/** Diekspor supaya lib/cron-olsera-revenue-recheck.ts bisa memakai aturan validasi laporan bulanan yang SAMA PERSIS saat me-refresh 4 laporan di akhir ronde re-check — jangan re-implementasi aturan ini di tempat lain. */
export function validateMonthlyReportPayload(type: string, normalized: any): { validated: boolean; note: string | null } {

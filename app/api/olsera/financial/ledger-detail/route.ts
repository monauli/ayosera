...
export async function GET(req: Request) { try { await guard(); const url = new URL(req.url); const period = validatePeriod(url.searchParams.get("year"), url.searchParams.get("month")); const accountCode = url.searchParams.get("accountCode") ?? ""; if (!/^\d{1,20}$/.test(accountCode)) return json({ status: "upstream-error", message: "Nomor akun tidak valid." }); const rawPayload = await getLedgerDetail(period, accountCode); return json({ status: "success", period, reportType: "ledger-detail", accountCode, normalizedPayload: normalizeLedgerDetailPayload(rawPayload, accountCode), rawPayload }); } catch (error) { return errorJson(error); } }

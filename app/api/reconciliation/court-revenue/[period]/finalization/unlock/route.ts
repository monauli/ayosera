...
export async function POST(request: Request, context: { params: Promise<{ period: string }> }) {
  try { const user = await requireModule("kunci-rekonsiliasi-omset"); const { period } = await context.params; if (!PERIOD.test(period)) return NextResponse.json({ error: "Format periode tidak valid." }, { status: 400, headers: NO_CACHE_HEADERS }); const payload = await request.json(); const lock = await unlockOmzetPeriodFinalization({ storeId: currentStoreId(), period, actor: user.id, expectedVersion: payload.version, reason: payload.reason }); return NextResponse.json({ data: await attachActorDisplayNames(lock) }, { headers: NO_CACHE_HEADERS }); }
